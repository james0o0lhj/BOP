...
p = {
    ...
    # bbox type. Options: 'modal', 'amodal'.  amodal=mask all    modal=mask only visible
    "bbox_type": "modal",
...
for scene_id in dp_split["scene_ids"]:
    ...
    for scene_view, inst_list in tqdm(scene_gt.items(), desc='Processing Scenes', total=len(scene_gt)):
        ...
        for idx, inst in enumerate(inst_list):
            ...
            annotation_info = pycoco_utils.create_annotation_info(
                ...
                binary_inst_mask_visib,
                bounding_box,
                mask_encoding_format="polygon",    # not rle
                tolerance=2,
                ignore=ignore_gt,
            )
